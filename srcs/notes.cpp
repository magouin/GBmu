...
{0b11100000, "LD (n), A", 1, {_, _, _, _}, std::bind(&Emulateur::ld, &emu, 0, (void *)NULL, &emu.regs.af.af.A, (struct s_params){MEM_gb, ADDR_x64, false, false, 1}, 3)},
{0b11100001, "POP HL", 0, {_, _, _, _}, std::bind(&Emulateur::pop, &emu, &emu.regs.hl.HL, (struct s_params){ADDR_x64, NO_PARAM, false, false, 2}, 3)},
{0b11100010, "LD (C), A", 0, {_, _, _, _}, std::bind(&Emulateur::ld, &emu, 0, &emu.regs.bc.bc.C, &emu.regs.af.af.A, (struct s_params){MEM_gb, ADDR_x64, false, false, 1}, 2)},
...
{0b11110010, "LD A, (C)", 0, {_, _, _, _}, std::bind(&Emulateur::ld, &emu, 0, &emu.regs.af.af.A, &emu.regs.bc.bc.C, (struct s_params){ADDR_x64, MEM_gb, false, false, 1}, 2)},
