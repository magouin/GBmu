...
void			Memory_controller::write_dma(uint8_t value)
{
	if (_emu.cgb.on) {
		...
		if (value >= 0x80 && value < 0xa0)
			memcpy(_emu.RAM + 0xfe00, (_video_ram_bank ? (_video_external_ram + (value - 0x80) * 256) : (_emu.RAM + value * 256)), 40 * 4);
		else if (value >= 0xa0 && value < 0xc0)
			memcpy(_emu.RAM + 0xfe00, ram_bank + (value - 0xa0) * 256, 40 * 4);
		else if (_working_ram_bank > 1 && value >= 0xd0 && value < 0xe0)
			memcpy(_emu.RAM + 0xfe00, _working_ram + (_working_ram_bank - 2) * 0x1000 + (value - 0xd0) * 256, 40 * 4);
		else
