...
bool		Memory_controller_MBC3::write_ROM_regs(uint8_t *addr, uint8_t value)
{
	...
	else if (addr - _emu.RAM < 0x4000)
	{
		if (!value)
			value = 1;
		_rom_bank_selected = value & 0x7f;
	}
	else if (addr - _emu.RAM < 0x6000)
	{
		if (value <= 3) {
			_ram_rtc_select = E_RAM;
			_ram_ext_work_bank_to_select = value;
		}
...
Memory_controller_MBC3::Memory_controller_MBC3(Emulateur &emu, size_t ram_size, bool debug): Memory_controller(emu, ram_size, debug)
{
	init(ram_size);
}
