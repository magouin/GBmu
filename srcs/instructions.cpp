...
void	Emulateur::ld(struct s_param *p1, struct s_param *p2, int8_t inc, int size)
{
	...
	get_param(p2);
	regs.HL += inc;
	_Mem_ctrl->mem_write(p1->val, _Mem_ctrl->mem_read(p2->val, size), size);
}
...
void	Emulateur::inc(struct s_param *p)
{
	...
	get_param(p);
	val = _Mem_ctrl->mem_read(p->val, p->size);
	if (p->size == 1)
	{
		regs.N = false;
		regs.HC = ((val & 0xf) == 0xf);
	...
	_Mem_ctrl->mem_write(p->val, val + 1, p->size);
}
...
{
	get_param(p);
	regs.A = regs.A & _Mem_ctrl->mem_read(p->val, 1);
	regs.F = FLAG_H;
...
void	Emulateur::add(struct s_param *p1, struct s_param *p2, int size)
{
	...
	get_param(p1);
	get_param(p2);
	v1 = _Mem_ctrl->mem_read(p1->val, p1->size);
	v2 = _Mem_ctrl->mem_read(p2->val, p2->size);
	...
	_Mem_ctrl->mem_write(p1->val, val, p1->size);
}
...
void	Emulateur::adc(struct s_param *p)
{
	...
	get_param(p);
	v1 = regs.A;
	v2 = _Mem_ctrl->mem_read(p->val, 1);
	carry = regs.CY;
	val = v1 + v2 + carry;
	regs.F = val ? 0 : FLAG_Z;
	regs.CY = (v1 + v2 + carry > 0xff);
	regs.HC = ((v1 & 0x0f) + (v2 & 0x0f) + carry > 0x0f);
	_Mem_ctrl->mem_write(&regs.A, val, 1);
}
...
void	Emulateur::sub(struct s_param *p)
{
	...
	regs.Z = (v1 == v2);
	_Mem_ctrl->mem_write(&regs.A, v1 - v2, 1);
}
...
{
	get_param(p);
	regs.PC += (int8_t)_Mem_ctrl->mem_read(p->val, 1);
}

void	Emulateur::ret()
{
	regs.PC = _Mem_ctrl->mem_read(_RAM + regs.SP, 2);
	regs.SP += 2;
}
...
{
	get_param(p);
	_Mem_ctrl->mem_write(p->val, _Mem_ctrl->mem_read(_RAM + regs.SP, 2), 2);
	regs.SP += 2;
}
...
void	Emulateur::get_param(struct s_param *p)
{
	...
	if (p->deref && p->s == 1)
		p->val = _RAM + 0xFF00 + _Mem_ctrl->mem_read(p->val, 1);
	else if (p->deref && p->s == 2)
		p->val = _RAM + _Mem_ctrl->mem_read(p->val, 2);
}

void	Emulateur::jp(struct s_param *p)
{
	get_param(p);
	regs.PC = _Mem_ctrl->mem_read(p->val, 2);
}

void	Emulateur::call(struct s_param *p)
{
	get_param(p);
	_Mem_ctrl->mem_write(_RAM + regs.SP - 2, regs.PC, 2);
	regs.PC = _Mem_ctrl->mem_read(p->val, 2);
	regs.SP -= 2;
}
...
void	Emulateur::push(struct s_param *p)
{
	get_param(p);
	regs.SP -= 2;
	_Mem_ctrl->mem_write(_RAM + regs.SP, _Mem_ctrl->mem_read(p->val, 2), 2);
}
...
void	Emulateur::ldhl(struct s_param *p)
{
	...
	get_param(p);
	regs.F = 0;
	e = (int8_t)_Mem_ctrl->mem_read(p->val, 1);
	regs.HL = regs.SP + e;
...
void	Emulateur::rlc(struct s_param *p)
{
	...
	get_param(p);
	val = _Mem_ctrl->mem_read(p->val, 1);
	tmp = val & 0x80;
	_Mem_ctrl->mem_write(p->val, (val << 1) | tmp, 1);
	regs.F = 0;
	regs.Z = !val;
	regs.CY = tmp;
}
...
void	Emulateur::rrc(struct s_param *p)
{
	...
	val = _Mem_ctrl->mem_read(p->val, 1);
	tmp = val & 1;
	_Mem_ctrl->mem_write(p->val, (val >> 1) | (tmp << 7), 1);
	regs.F = 0;
	regs.Z = !val;
	regs.CY = tmp;
}
...
void	Emulateur::rl(struct s_param *p)
{
	...
	val = (val << 1) | tmp;
	regs.Z = !val;
	_Mem_ctrl->mem_write(p->val, val, 1);
}
...
void	Emulateur::sla(struct s_param *p)
{
	...
	regs.Z = !((val & 0x7f));
	regs.CY = val >> 7;
	_Mem_ctrl->mem_write(p->val, (val << 1), 1);
}
...
void	Emulateur::sra(struct s_param *p)
{
	...
	regs.Z = !(val >> 1);
	regs.CY = val & 1;
	_Mem_ctrl->mem_write(p->val, val >> 1 | (val & 0x80), 1);
}
...
void	Emulateur::_swap(struct s_param *p)
{
	...
	regs.F = 0;
	regs.Z = !val;
	_Mem_ctrl->mem_write(p->val, val << 4 | val >> 4, 1);
}
...
void	Emulateur::bit(struct s_param *p, uint8_t bit)
{
	...
	regs.HC = true;
	regs.N = false;
	regs.Z = (_Mem_ctrl->mem_read(p->val, 1) & (1 << bit)) ? 0 : 1;
}

void	Emulateur::res(struct s_param *p, uint8_t bit)
{
	get_param(p);
	_Mem_ctrl->mem_write(p->val, _Mem_ctrl->mem_read(p->val, 1) & ~(1 << bit), 1);
}
